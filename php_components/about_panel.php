<div class="container">
    <div class="row vertical-center-100">
        <div class="col-md-6 col-md-offset-3 col-sm-6 col-sm-offset-3 col-xs-9 col-xs-offset-1">
            <div class="panel panel-inverse">

                <!-- panel header -->
                <div class="panel-heading">
                    <strong>About the server</strong>
                </div>

                <!-- panel body -->
                <div class="panel-body">
                  <strong>Server patch:</strong><span> 3.3.5a (woltk)</span>
                  </BR>
                  <strong>Server rates:</strong><span> 5x experience, 2x drop, 2x honor</span>
                  </BR>
                  <strong>Realmlist (realmlist.wtf):</strong><span> set ''</span>
                </div>

                <!-- panel footer -->
                <div class="panel-footer">
                    <span>Problems with something? <a href="#">Contact us</a></span>
                </div>

            </div>
        </div>
    </div>
</div>
